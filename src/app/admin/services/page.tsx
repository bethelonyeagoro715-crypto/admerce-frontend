...
function normalizeServices(data: unknown): AdminService[] {
  if (Array.isArray(data)) return data as AdminService[];
  if (data && typeof data === 'object') {
    const obj = data as Record<string, unknown>;
    if (Array.isArray(obj.services)) return obj.services as AdminService[];
    if (Array.isArray(obj.data)) return obj.data as AdminService[];
    if (Array.isArray(obj.results)) return obj.results as AdminService[];
  }
  return [];
}
...
export default function AdminServicesPage() {
  ...
  const loadServices = async (page = 0, query = searchQuery) => {
    setCurrentPage(page);
    setIsLoading(true);
    ...
    try {
      ...
      setErrorMessage('Failed to load services: ' + (err instanceof Error ? err.message : ''));
      setServices([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const deleteService = async (serviceId: string, title: string) => {
    if (!window.confirm(`Delete "${title}"? This cannot be undone.`)) return;
    try {
      await api.adminDeleteService(serviceId);
      await loadServices(currentPage, searchQuery);
    } catch (err) {
      alert('Delete failed: ' + (err instanceof Error ? err.message : ''));
    }
  };

  const nextPage = () => loadServices(currentPage + 1, searchQuery);
  const prevPage = () => {
    if (currentPage > 0) loadServices(currentPage - 1, searchQuery);
  };
  ...
  return (
    <main style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <button onClick={() => loadServices(0, searchQuery)} style={styles.iconBtn} title="Refresh">
      ...
      <div style={styles.content}>
        {isLoading ? (
          ...
          <div style={styles.center}>
            ...
            <p>{errorMessage}</p>
            <button onClick={() => loadServices(0, searchQuery)} style={styles.retryBtn}>
              Retry
            </button>
          </div>
        ) : services.length === 0 ? (
          ...
          <div style={styles.tableWrapper}>
            ...
            {services.map((service) => (
              <div key={service.service_id} style={styles.row}>
                <span style={{ flex: 2, fontWeight: 600, color: '#1A1A1A' }}>
                  {service.title || '—'}
                </span>
                <span style={{ flex: 1 }}>{formatPrice(service.price)}</span>
                <span style={{ flex: 1 }}>{service.category || '—'}</span>
                <span style={{ flex: 1 }}>{formatDate(service.created_at)}</span>
                <span style={{ flex: 1, display: 'flex', justifyContent: 'center' }}>
                  <button
                    onClick={() => deleteService(service.service_id, service.title || '')}
      ...
      {!isLoading && !errorMessage && (
        <div style={styles.pagination}>
          ...
          >
            Next
            <MdKeyboardArrowRight size={20} color={services.length < limit ? '#ccc' : '#0504AA'} />
          </button>
        </div>
      )}
