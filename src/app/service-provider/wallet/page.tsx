...
export default function ServiceProviderWalletPage() {
  ...
  const handleTopUp = async () => {
    ...
    if (!email?.includes('@')) {
      alert('Email missing — update your profile first.');
      return;
    }
    setPaying(true);
    try {
    ...
    <div style={css.root}>
      ...
      <div style={css.hero}>
        ...
        <div style={css.topBar}>
          ...
          <span style={css.heroTitle}>Earnings Wallet</span>
          <button
            style={css.ghostBtn}
        ...
        <div style={css.balanceBlock}>
          ...
          <div style={css.balSub}>Admerce Wallet · Service Provider</div>
        </div>
      ...
      <div style={css.sheet}>
        <div style={css.actionsGrid}>
          {([
            ...
            {
              icon: <MdArrowUpward size={20} color="#fff" />,
              label: 'Withdraw',
              bg: '#7C3AED',
              action: () => router.push('/service-provider/wallet/withdraw'),
            },
            {
              icon: <MdHistory size={20} color="#fff" />,
              label: 'History',
              bg: '#0891B2',
              action: () => router.push('/service-provider/wallet/history'),
            },
            {
        ...
        <div style={css.txnList}>
          {transactions.length === 0 ? (
            <div style={css.empty}>
              ...
              <p style={{ margin: '12px 0 4px', fontWeight: 600, color: '#6366F1' }}>
                No transactions yet
              </p>
            ...
            transactions.map((txn, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...css.txnCard,
                    borderLeft: `3px solid ${isCredit ? '#4CDE80' : '#FF5757'}`,
        ...
        <div style={css.overlay} onClick={() => setShowTopUp(false)}>
          <div style={css.bottomSheet} onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={handleTopUp} style={css.payBtn} disabled={paying}>
              {paying
                ? 'Opening Paystack…'
                : `Pay ${amount ? fmt(parseFloat(amount) || 0) : ''} with Paystack`}
            </button>
            <button style={css.cancelBtn} onClick={() => setShowTopUp(false)}>
              Cancel
            </button>
          </div>
        </div>
